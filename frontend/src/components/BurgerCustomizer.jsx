...
export default function BurgerCustomizer() {
  ...
  const addToCart = async () => {
    const name = `${patty} Burger on ${bun} (${toppings.join(', ') || 'No Toppings'})${
        combo !== 'Nothing' ? ' - ' + combo : ''
    }`;
      ...
      <div className="p-4">
        ...
        <div className="row">
          <div className="col-md-8">
            <div className="mb-3">
              <label>Bun:</label>
              <select className="form-select" value={bun} onChange={(e) => setBun(e.target.value)}>
                {BUNS.map((b) => (
            ...
            <div className="mb-3">
              <label>Patty:</label>
              <select className="form-select" value={patty} onChange={(e) => setPatty(e.target.value)}>
                {PATTIES.map((p) => (
            ...
            <div className="mb-3">
              <label>Combo:</label>
              <select className="form-select" value={combo} onChange={(e) => setCombo(e.target.value)}>
                {COMBOS.map((c) => (
